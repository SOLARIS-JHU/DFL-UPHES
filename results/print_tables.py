...
def generate_latex_tables(df, output_dir):
    ...
    # Separate methods into recursive and non-recursive
    recursive_methods = [m for m in method_stats.index if 'NoRec' not in m]
    norec_methods = [m for m in method_stats.index if 'NoRec' in m]
    
    # Sort each group
    recursive_methods = sorted(recursive_methods)
...
"""
    ...
    # Performance Ranking Table - also group NoRec methods
    ranking_stats = method_stats.sort_values(('Ex_post_Profit', 'mean'), ascending=False)
    
    # Separate ranking into recursive and non-recursive
    recursive_ranking = [m for m in ranking_stats.index if 'NoRec' not in m]
    norec_ranking = [m for m in ranking_stats.index if 'NoRec' in m]
    
    # Sort by profit within each group
    recursive_ranking = sorted(recursive_ranking, 
...
"""
    ...
    # Add recursive methods
    for i, method in enumerate(recursive_ranking, 1):
