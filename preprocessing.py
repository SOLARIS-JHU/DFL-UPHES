...
def read_da_price(date, file_path="./Data/Day-ahead Prices_202301010000-202401010000-2.csv"):
    ...
    tensor_data = torch.tensor(filtered_data.values, dtype=torch.float)
...
def plot_poly_UPC_boundaries(boundaries_neg, boundaries_pos, h_poly_fit, poly_neg_min_fit, poly_neg_max_fit, poly_pos_min_fit, poly_pos_max_fit):
    ...
    neg_min_line = np.polyval(poly_neg_min_fit, h_poly_fit)
    neg_max_line = np.polyval(poly_neg_max_fit, h_poly_fit)
    pos_min_line = np.polyval(poly_pos_min_fit, h_poly_fit)
    pos_max_line = np.polyval(poly_pos_max_fit, h_poly_fit)
...
poly_neg_min_fit = torch.tensor(poly_neg_min_fit, dtype=torch.float32)
poly_neg_max_fit = torch.tensor(poly_neg_max_fit, dtype=torch.float32)
poly_pos_min_fit = torch.tensor(poly_pos_min_fit, dtype=torch.float32)
poly_pos_max_fit = torch.tensor(poly_pos_max_fit, dtype=torch.float32)
...
# test
print(h_to_v_low_fitted(99))
print(h_to_v_low_fitted(50))
...
if __name__ == '__main__':
    ...
    coefs_tur = torch.tensor(results_turbine[7], dtype=torch.float32)
    intercept_tur = torch.tensor(results_turbine[8], dtype=torch.float32)
    coefs_pump = torch.tensor(results_pump[7], dtype=torch.float32)
    intercept_pump = torch.tensor(results_pump[8], dtype=torch.float32)

'''
def predict_q_poly(p, h, model_tur, model_pump):
    """Predict flow (q) value using the fitted model directly."""
    # Create input array
    input_features = np.array([[p, h]])
    
    # Use model's predict method to make predictions
    if p > 0:
        q_predicted = model_tur.predict(input_features)
    elif p < 0:
        q_predicted = model_pump.predict(input_features)
...
with open('preprocess.pkl', 'wb') as f:
    pickle.dump((h_fit, neg_min_fit, neg_max_fit, pos_min_fit, pos_max_fit, h_v_poly, DA_price_hour, DA_price_quarter, h_to_v_low_fitted, predict_q_poly, neg_min, neg_max, pos_min, pos_max, prepare_and_fit_model, get_UPC_bound, LR_UPC_bound), f)
