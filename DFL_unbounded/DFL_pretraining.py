...
def generate_cross_database_comparison():
    """Generate comparative analysis between the three euclidean databases."""
    try:
        # Check if all three benchmark files exist
        euclidean_global_benchmark = Path("./trained_models/euclidean_global_linear/pretraining_benchmarks.csv")
        euclidean_neural_benchmark = Path("./trained_models/euclidean_neural_network/pretraining_benchmarks.csv")
        ...
        if not euclidean_neural_benchmark.exists():
            missing_files.append("euclidean_neural_network")
        ...
        df_neural = pd.read_csv(euclidean_neural_benchmark)
        df_piecewise = pd.read_csv(euclidean_piecewise_benchmark)
        
        # Add source column
        df_global['Source'] = 'euclidean_global_linear'
        df_neural['Source'] = 'euclidean_neural_network'
        df_piecewise['Source'] = 'euclidean_piecewise'
        
        # Combine data
        df_combined = pd.concat([df_global, df_neural, df_piecewise])
        ...
        neural_data = avg_profit[avg_profit['Source'] == 'euclidean_neural_network']
        ...
        neural_y = [neural_data[neural_data['Config'] == config]['Simulated_Profit'].values[0]
                    if len(neural_data[neural_data['Config'] == config]) > 0 else 0
                    for config in configs]
        ...
        # Plot bars
        plt.bar(x - width, global_y, width, label='Global Linear', color='blue')
        plt.bar(x, neural_y, width, label='Neural Network', color='green')
        plt.bar(x + width, piecewise_y, width, label='Piecewise', color='red')
        ...
        neural_time = avg_time[avg_time['Source'] == 'euclidean_neural_network']
        ...
        neural_time_y = [neural_time[neural_time['Config'] == config]['Training_Time_Seconds'].values[0]
                         if len(neural_time[neural_time['Config'] == config]) > 0 else 0
                         for config in configs]
        ...
        # Plot bars
        plt.bar(x - width, global_time_y, width, label='Global Linear', color='blue')
        plt.bar(x, neural_time_y, width, label='Neural Network', color='green')
        plt.bar(x + width, piecewise_time_y, width, label='Piecewise', color='red')
        ...
        neural_best = avg_profit[avg_profit['Source'] == 'euclidean_neural_network'].loc[
            avg_profit[avg_profit['Source'] == 'euclidean_neural_network']['Simulated_Profit'].idxmax()
        ]
        ...
        best_profits = [
            ...
            ('euclidean_neural_network', neural_best['Simulated_Profit']),
        ...
        elif overall_best_db == 'euclidean_neural_network':
            overall_best_config = neural_best
        else:
        ...
        with open(comparison_dir / "database_comparison.txt", 'w') as f:
            ...
            f.write("Euclidean Neural Network Database:\n")
            f.write(f"  Best Configuration: {neural_best['Config']}\n")
            f.write(f"  Average Simulated Profit: {neural_best['Simulated_Profit']:.2f}\n")
            neural_time_val = neural_time[neural_time['Config'] == neural_best['Config']]['Training_Time_Seconds'].values
            if len(neural_time_val) > 0:
                f.write(f"  Average Training Time: {neural_time_val[0]:.2f} seconds\n\n")
            else:
                f.write(f"  Average Training Time: N/A\n\n")
...
def train_single_model(source_name, file_path, architecture, num_layers, max_iterations, date_str, date_data, start_timestamp):
    ...
    try:
        ...
        weight_network = BoundedLogWeightPredictor(
            ...
            init_w_q=0.02,
            init_w_h=0.1,
            w_p_min=1e-6,  # Minimum value for w_p
            w_p_max=1e6,   # Maximum value for w_p
            w_q_min=1e-6,  # Minimum value for w_q
            w_q_max=1e6,   # Maximum value for w_q
            w_h_min=1e-6,  # Minimum value for w_h
            w_h_max=1e6    # Maximum value for w_h
        ).to(device)
...
def retry_failed_models():
    ...
    database_sources = ['euclidean_global_linear', 'euclidean_neural_network', 'euclidean_piecewise']
