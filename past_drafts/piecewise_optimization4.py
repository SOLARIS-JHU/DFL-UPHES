...
# %% Piecewise MILP Optimizer with SOS2 constraints - Compatible version
# %% Import libraries
import torch
import numpy as np
import cvxpy as cp
...
class PiecewiseMILPOptimizer:
    ...
    def _build_model(self):
        ...
        for t in range(T):
            ...
            for i in range(self.num_segments_h):
                for j in range(self.num_segments_p_pump):
                    ...
                    for c in range(4):
                        ...
                        pump_h_expr.add(self.lambda_pump[t, i, j, c] * corners_h[c])
                    
                    # Add SOS2 constraints for proper bilinear interpolation within the active quadrilateral
                    # These constraints will be automatically activated when delta_pump[t,i,j] = 1
                    # We use lambda variables only for the current quadrilateral (t,i,j)
                    
                    # Create SOS2 constraints for each edge of the quadrilateral
                    self.model.addConstr(self.lambda_pump[t, i, j, 0] + self.lambda_pump[t, i, j, 1] <= self.delta_pump[t, i, j], 
                                    name=f"pump_edge01_{t}_{i}_{j}")
                    self.model.addConstr(self.lambda_pump[t, i, j, 1] + self.lambda_pump[t, i, j, 2] <= self.delta_pump[t, i, j], 
                                    name=f"pump_edge12_{t}_{i}_{j}")
                    self.model.addConstr(self.lambda_pump[t, i, j, 2] + self.lambda_pump[t, i, j, 3] <= self.delta_pump[t, i, j], 
                                    name=f"pump_edge23_{t}_{i}_{j}")
                    self.model.addConstr(self.lambda_pump[t, i, j, 3] + self.lambda_pump[t, i, j, 0] <= self.delta_pump[t, i, j], 
                                    name=f"pump_edge30_{t}_{i}_{j}")
            ...
            for i in range(self.num_segments_h):
                for j in range(self.num_segments_p_turbine):
                    ...
                    for c in range(4):
                        ...
                        turbine_h_expr.add(self.lambda_turbine[t, i, j, c] * corners_h[c])
                    
                    # Add SOS2 constraints for proper bilinear interpolation within the active quadrilateral
                    # These constraints will be automatically activated when delta_turbine[t,i,j] = 1
                    # We use lambda variables only for the current quadrilateral (t,i,j)
                    
                    # Create SOS2 constraints for each edge of the quadrilateral
                    self.model.addConstr(self.lambda_turbine[t, i, j, 0] + self.lambda_turbine[t, i, j, 1] <= self.delta_turbine[t, i, j], 
                                    name=f"turbine_edge01_{t}_{i}_{j}")
                    self.model.addConstr(self.lambda_turbine[t, i, j, 1] + self.lambda_turbine[t, i, j, 2] <= self.delta_turbine[t, i, j], 
                                    name=f"turbine_edge12_{t}_{i}_{j}")
                    self.model.addConstr(self.lambda_turbine[t, i, j, 2] + self.lambda_turbine[t, i, j, 3] <= self.delta_turbine[t, i, j], 
                                    name=f"turbine_edge23_{t}_{i}_{j}")
                    self.model.addConstr(self.lambda_turbine[t, i, j, 3] + self.lambda_turbine[t, i, j, 0] <= self.delta_turbine[t, i, j], 
                                    name=f"turbine_edge30_{t}_{i}_{j}")
            ...
            # Ensure consistency with head variable (only needed for active modes)
            self.model.addConstr(self.h[t] == pump_h_expr + turbine_h_expr, name=f"h_combined_{t}")
