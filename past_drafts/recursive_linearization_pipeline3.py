...
if __name__ == "__main__":
    ...
    # Initialize v_low_init after loading the fitted function
    v_low_init = torch.tensor(310528.5000, device=device)  # Initial lower reservoir volume
    print(f"head_min: {head_min}, head_max: {head_max}, max_vol_up: {max_vol_up}, min_vol_low: {min_vol_low}, ramp_up: {ramp_up}, ramp_down: {ramp_down}")
    print(f"target_head: {target_head}, target_vol_low: {target_vol_low}, head_init: {head_init}, v_low_init: {v_low_init}")
...
def train_recursive_linearization(weight_network, params, optimizer_layer, regression_layer, 
    ...
    Reduced output version - only shows progress and errors.
    """
    ...
    for epoch in range(num_epochs):
        try:
            ...
            # Print progress every 10 epochs or on key events
            if epoch % 10 == 0 or epoch == num_epochs - 1:
                print(f"Epoch {epoch}: Expected Profit = {optimized_profit.item():.2f}, Ex-post Profit = {simulated_profit.item():.2f}")
            ...
            if simulated_profit.item() > best_profit:
                ...
                patience_counter = 0
                if epoch % 10 != 0:  # Only print if not already printed above
                    print(f"Epoch {epoch}: New best ex-post profit: {best_profit:.2f}")
            else:
                patience_counter += 1
    ...
    print(f"Best Ex-post Profit: {best_profit:.2f}")
    
    return weight_network, history

def plot_profit_vs_epoch(history):
    """
    Generate a simple profit vs epoch plot showing Expected Profit and Ex-post Profit curves.
    Save as SVG format.
    """
    # Create figure
    plt.figure(figsize=(10, 6))
    
    # Plot both profit curves
    plt.plot(history['epoch'], history['profit'], 'b-', linewidth=2, label='Expected Profit', marker='o', markersize=4)
    plt.plot(history['epoch'], history['simulated_profit'], 'r-', linewidth=2, label='Ex-post Profit', marker='s', markersize=4)
    
    # Customize the plot
    plt.title('Profit vs Epoch', fontsize=14, fontweight='bold')
...
def pretraining_with_grid_search():
    ...
    database_sources = {
        'SOS2': './Data/SOS2_database.csv',
        'GlobalLinear': './Data/global_linear_database.csv'
    }
    
    # Define grid search parameters
    architectures = ['LSTM','RNN']
    num_layers_list = [1, 2, 3]
    max_iterations_list = [3, 5, 7] # list(range(1, 16))
    ...
    for source_name, file_path in database_sources.items():
        ...
        root_dir = Path(f"./trained_models_euclidean/{source_name}")
        ...
        for architecture, num_layers, max_iterations in itertools.product(
            ...
            print(f"\nStarting training for configuration: {config_name} with {source_name} data")
            ...
            for date_idx, (date_str, date_data) in enumerate(historical_data.items()):
                config_counter += 1
                print(f"[{config_counter}/{total_configs}] Training {source_name} data for date: {date_str} with {config_name}")
                ...
                try:
                    ...
                    with open(date_dir / "training_history.json", 'w') as f:
                        ...
                        simplified_history = {
                            ...
                            'expected_profit': [float(x) for x in history['profit']],
                            'ex_post_profit': [float(x) for x in history['simulated_profit']],
                    ...
                    # Generate simplified profit plot and save as SVG
                    plot_profit_vs_epoch(history)
                    plt.savefig(date_dir / "profit_vs_epoch.svg", format='svg', dpi=300, bbox_inches='tight')
                    plt.close()
                    
                    # Get final metrics from the last epoch
                    last_idx = len(history['epoch']) - 1
                    final_expected_profit = float(history['profit'][last_idx])
                    final_ex_post_profit = float(history['simulated_profit'][last_idx])
                    ...
                    # Find best epoch (maximum ex-post profit)
                    ...
                    # Append benchmark data with updated column names
                    with open(benchmark_file, 'a', newline='') as f:
                        ...
                        writer.writerow([
                            ...
                            f"{final_expected_profit:.2f}", f"{final_ex_post_profit:.2f}",
                    ...
                    print(f"  Final expected profit: {final_expected_profit:.2f}")
                    print(f"  Final ex-post profit: {final_ex_post_profit:.2f}")
...
def generate_benchmark_summary(benchmark_file, source_name):
    ...
    try:
        ...
        summary_dir = Path(f"./trained_models_euclidean/{source_name}/summary")
        ...
        plt.tight_layout()
        plt.savefig(summary_dir / "training_time_by_config.svg", format='svg', dpi=300, bbox_inches='tight')
        plt.close()
        
        # 2. Profit comparison by configuration (updated column names)
        plt.figure(figsize=(12, 8))
        avg_profit = df.groupby(['Architecture', 'Num_Layers', 'Max_Iterations'])[
            ['Expected_Profit', 'Ex_post_Profit']
        ].mean().reset_index()
        ...
        width = 0.35
        plt.bar(x - width/2, avg_profit['Expected_Profit'], width, label='Expected Profit', color='blue')
        plt.bar(x + width/2, avg_profit['Ex_post_Profit'], width, label='Ex-post Profit', color='red')
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(summary_dir / "profit_by_config.svg", format='svg', dpi=300, bbox_inches='tight')
        plt.close()
        
        # 3. Find best configurations (updated column names)
        best_expected = avg_profit.loc[avg_profit['Expected_Profit'].idxmax()]
        best_ex_post = avg_profit.loc[avg_profit['Ex_post_Profit'].idxmax()]
        ...
        with open(summary_dir / "benchmark_summary.txt", 'w') as f:
            ...
            f.write("Best Configuration for Expected Profit:\n")
            f.write(f"  Architecture: {best_expected['Architecture']}\n")
            f.write(f"  Number of Layers: {best_expected['Num_Layers']}\n")
            f.write(f"  Max Iterations: {best_expected['Max_Iterations']}\n")
            f.write(f"  Average Expected Profit: {best_expected['Expected_Profit']:.2f}\n\n")
            
            f.write("Best Configuration for Ex-post Profit:\n")
            f.write(f"  Architecture: {best_ex_post['Architecture']}\n")
            f.write(f"  Number of Layers: {best_ex_post['Num_Layers']}\n")
            f.write(f"  Max Iterations: {best_ex_post['Max_Iterations']}\n")
            f.write(f"  Average Ex-post Profit: {best_ex_post['Ex_post_Profit']:.2f}\n\n")
        ...
        best_config = {
            'architecture': best_ex_post['Architecture'],
            'num_layers': int(best_ex_post['Num_Layers']),
            'max_iterations': int(best_ex_post['Max_Iterations']),
            'average_ex_post_profit': float(best_ex_post['Ex_post_Profit'])
        }
...
def generate_cross_database_comparison():
    ...
    try:
        ...
        sos2_benchmark = Path("./trained_models_euclidean/SOS2/pretraining_benchmarks.csv")
        global_benchmark = Path("./trained_models_euclidean/GlobalLinear/pretraining_benchmarks.csv")
        ...
        comparison_dir = Path("./trained_models_euclidean/comparison")
...
def retrain_failed_models():
    """
    Retrain all the failed models from the error log with improved error handling.
    Extracts all failed configurations from the error log and retrains them.
    """
    # Define all failed configurations from the error log
    failed_configurations = [
        # Date: 2021-06-03
        {'date': '2021-06-03', 'architecture': 'LSTM', 'num_layers': 1, 'max_iterations': 3},
        {'date': '2021-06-03', 'architecture': 'LSTM', 'num_layers': 1, 'max_iterations': 7},
        {'date': '2021-06-03', 'architecture': 'RNN', 'num_layers': 1, 'max_iterations': 5},
        {'date': '2021-06-03', 'architecture': 'RNN', 'num_layers': 1, 'max_iterations': 7},
        {'date': '2021-06-03', 'architecture': 'RNN', 'num_layers': 2, 'max_iterations': 5},
        
        # Date: 2023-08-19
        {'date': '2023-08-19', 'architecture': 'LSTM', 'num_layers': 1, 'max_iterations': 5},
        {'date': '2023-08-19', 'architecture': 'LSTM', 'num_layers': 1, 'max_iterations': 7},
        {'date': '2023-08-19', 'architecture': 'LSTM', 'num_layers': 2, 'max_iterations': 3},
        {'date': '2023-08-19', 'architecture': 'LSTM', 'num_layers': 2, 'max_iterations': 7},
        {'date': '2023-08-19', 'architecture': 'LSTM', 'num_layers': 3, 'max_iterations': 3},
        {'date': '2023-08-19', 'architecture': 'LSTM', 'num_layers': 3, 'max_iterations': 7},
        {'date': '2023-08-19', 'architecture': 'RNN', 'num_layers': 1, 'max_iterations': 3},
        {'date': '2023-08-19', 'architecture': 'RNN', 'num_layers': 1, 'max_iterations': 5},
        {'date': '2023-08-19', 'architecture': 'RNN', 'num_layers': 3, 'max_iterations': 3},
        
        # Date: 2021-06-21
        {'date': '2021-06-21', 'architecture': 'LSTM', 'num_layers': 2, 'max_iterations': 3},
        {'date': '2021-06-21', 'architecture': 'LSTM', 'num_layers': 3, 'max_iterations': 3},
        {'date': '2021-06-21', 'architecture': 'RNN', 'num_layers': 2, 'max_iterations': 7},
    ]
    
    # Load historical data specifically for SOS2 database
    historical_data = load_data_for_pretraining('./Data/SOS2_database.csv', 'SOS2')
    
    if not historical_data:
        print("Error: Could not load SOS2 data. Exiting...")
        return
    ...
    root_dir = Path("./trained_models_euclidean/SOS2")
    
    # Benchmark file for updating
    benchmark_file = root_dir / "pretraining_benchmarks.csv"
    
    print(f"Retraining {len(failed_configurations)} failed configurations...")
    
    for i, config in enumerate(failed_configurations):
        ...
        print(f"\n{'='*60}")
        print(f"[{i+1}/{len(failed_configurations)}] Retraining: {config_name} for date: {date_str}")
        print(f"{'='*60}")
        
        # Check if we have data for this date
        if date_str not in historical_data:
            print(f"Warning: No data available for date {date_str}. Skipping...")
            continue
        ...
        # Check if model already exists (skip if successful model is already there)
        if (date_dir / "best_model.pt").exists() and (date_dir / "model.pt").exists():
            print(f"Model already exists for {config_name} on {date_str}. Skipping...")
            continue
        
        # Check if model already exists (skip if successful model is already there)
        if (date_dir / "best_model.pt").exists() or (date_dir / "model.pt").exists():
            print(f"Model already exists for {config_name} on {date_str}. Skipping...")
            continue
        ...
        try:
            ...
            # Train the network with same parameters but increased patience and epochs
            trained_network, history = train_recursive_linearization(
                weight_network=weight_network,
                params=params,
                optimizer_layer=optimizer_layer,
            ...
            # Check if training was successful and history is populated
            if not history or 'epoch' not in history or len(history['epoch']) == 0:
                print(f"Error: Training failed - no history data generated for {config_name} on {date_str}")
                continue
            
            # Additional safety checks for history data
            if ('profit' not in history or len(history['profit']) == 0 or
                'simulated_profit' not in history or len(history['simulated_profit']) == 0):
                print(f"Error: Training failed - incomplete history data for {config_name} on {date_str}")
                continue
            ...
            # Get final metrics from the last epoch with safety checks
            last_idx = len(history['epoch']) - 1
            
            # Ensure the index is valid for all required lists
            if (last_idx < len(history['profit']) and 
                last_idx < len(history['simulated_profit']) and
                last_idx < len(history['SI_penalty']) and
                last_idx < len(history['volume_penalty']) and
                last_idx < len(history['operating_cost'])):
                ...
                print(f"Warning: History data length mismatch for {config_name} on {date_str}")
                print(f"  epochs: {len(history['epoch'])}, profit: {len(history['profit'])}")
                print(f"  simulated_profit: {len(history['simulated_profit'])}")
                # Use the last available values
                final_expected_profit = float(history['profit'][-1]) if history['profit'] else 0.0
                final_ex_post_profit = float(history['simulated_profit'][-1]) if history['simulated_profit'] else 0.0
                final_si_penalty = float(history['SI_penalty'][-1]) if history['SI_penalty'] else 0.0
                final_volume_penalty = float(history['volume_penalty'][-1]) if history['volume_penalty'] else 0.0
                final_operating_cost = float(history['operating_cost'][-1]) if history['operating_cost'] else 0.0
            
            # Find best epoch (maximum ex-post profit) with safety check
            if history['simulated_profit']:
                best_epoch_idx = np.argmax(history['simulated_profit'])
                best_epoch = history['epoch'][best_epoch_idx]
            else:
                best_epoch = 0
            ...
            print(f"Retraining completed for {date_str} with {config_name}:")
            print(f"  Training time: {training_time:.2f} seconds")
            ...
            print(f"Error retraining {date_str} with {config_name}: {e}")
            print(traceback.format_exc())
            
            # Log the error with more details
            with open(root_dir / "retrain_error_log.txt", 'a') as f:
                f.write(f"\n[{datetime.now()}] Error retraining {date_str} with {config_name}:\n")
    ...
    print("\nRetraining completed for all failed models.")
    print("Models saved to original directory structure: ./trained_models_euclidean/SOS2/{config_name}/{date}/")
    print("Check the retrain_error_log.txt file for any remaining issues.")

# Alternative function to retrain with more conservative settings
def retrain_failed_models_conservative():
    """
    Retrain failed models with more conservative settings to avoid memory issues.
    Uses smaller networks and fewer iterations. Saves to original directory structure.
    """
    # Same failed configurations as above
    failed_configurations = [
        # Date: 2021-06-03
        {'date': '2021-06-03', 'architecture': 'LSTM', 'num_layers': 1, 'max_iterations': 3},
        {'date': '2021-06-03', 'architecture': 'LSTM', 'num_layers': 1, 'max_iterations': 7},
    ...
    # Root directory for saving models (use original structure)
    root_dir = Path("./trained_models_euclidean/SOS2")
    ...
    for i, config in enumerate(failed_configurations):
        ...
        architecture = config['architecture']
        num_layers = config['num_layers']
        max_iterations = min(config['max_iterations'], 5)  # Cap at 5 iterations
        
        config_name = f"{architecture}_{num_layers}layer_{max_iterations}iter"
        
        print(f"\n[{i+1}/{len(failed_configurations)}] Conservative retraining: {config_name} for {date_str}")
        ...
        try:
            # Use smaller, more conservative network settings
            weight_network = BoundedLogWeightPredictor(
                input_size=4,
                hidden_size=64,  # Reduced from 128
                num_layers=min(num_layers, 2),  # Cap at 2 layers
                dropout=0.1,     # Reduced dropout
                time_horizon=params.time_horizon,
                archetype=architecture,
                init_w_p=0.1,    # Smaller initial weights
                init_w_q=0.01,
                init_w_h=0.05,
                w_p_min=0.01,    # Tighter bounds
                w_p_max=10.0,   
                w_q_min=0.001,
                w_q_max=1.0,
                w_h_min=0.01,
            ...
            trained_network, history = train_recursive_linearization(
                ...
                max_iterations=max_iterations,
                penalty_growth_rate=1.1  # Smaller growth rate
            )
            
            training_time = time.time() - start_time
            
            # Same safety checks as the main function
            if not history or 'epoch' not in history or len(history['epoch']) == 0:
                print(f"Error: Training failed - no history data generated")
                continue
            
            if ('profit' not in history or len(history['profit']) == 0 or
                'simulated_profit' not in history or len(history['simulated_profit']) == 0):
                print(f"Error: Training failed - incomplete history data")
                continue
            
            # Save results (use original naming convention)
            torch.save(trained_network.state_dict(), date_dir / "model.pt")
            torch.save(trained_network.state_dict(), date_dir / "best_model.pt")
            
            # Get final metrics with safety checks
            last_idx = len(history['epoch']) - 1
            final_expected_profit = float(history['profit'][last_idx]) if last_idx < len(history['profit']) else 0.0
            final_ex_post_profit = float(history['simulated_profit'][last_idx]) if last_idx < len(history['simulated_profit']) else 0.0
            final_si_penalty = float(history['SI_penalty'][last_idx]) if last_idx < len(history['SI_penalty']) else 0.0
            final_volume_penalty = float(history['volume_penalty'][last_idx]) if last_idx < len(history['volume_penalty']) else 0.0
            final_operating_cost = float(history['operating_cost'][last_idx]) if last_idx < len(history['operating_cost']) else 0.0
            
            best_epoch = np.argmax(history['simulated_profit']) if history['simulated_profit'] else 0
            
            # Append to benchmark
            with open(benchmark_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    architecture, min(num_layers, 2), max_iterations, date_str,
                    f"{training_time:.2f}", len(history['epoch']), best_epoch,
                    f"{final_expected_profit:.2f}", f"{final_ex_post_profit:.2f}",
                    f"{final_si_penalty:.2f}", f"{final_volume_penalty:.2f}",
                    f"{final_operating_cost:.2f}", datetime.now().strftime("%Y%m%d_%H%M%S")
                ])
            
            print(f"Conservative training completed successfully")
            print(f"  Final ex-post profit: {final_ex_post_profit:.2f}")
            
        except Exception as e:
            print(f"Error in conservative retraining: {e}")
            continue
    
    print("\nConservative retraining completed.")
    print("Models saved to original directory structure: ./trained_models_euclidean/SOS2/{config_name}/{date}/")

# Call the main function to retrain the failed models
if __name__ == "__main__":
    # First try the main retraining function
    print("Starting main retraining...")
    retrain_failed_models()
    
    # If you still have issues, uncomment the line below to try conservative settings
    # print("\nStarting conservative retraining...")
    # retrain_failed_models_conservative()
...
def find_closest_date(new_price, historical_data=None):
    """
    Find the date in Belgium historical data with the most similar price signal.
    Returns the matched training date for model lookup.
    
    Args:
        new_price: Tensor of shape [24] with hourly prices
        historical_data: Dictionary of historical training data (used for validation)
        
    Returns:
        str: Date string of the matched training date for model lookup
    ...
    belgium_file_path = "./Data/Belgium_historical_data.csv"
    
    try:
        # Read the Belgium CSV file
        df = pd.read_csv(belgium_file_path)
        
        closest_belgium_date = None
        matched_training_date = None
        min_distance = float('inf')
        
        # Process each row in the Belgium data
        for _, row in df.iterrows():
            belgium_date_str = row['date']
            training_date_str = row['matched_from_2024_date']
            prices_str = row['prices_hourly']
            
            try:
                # Parse the comma-separated price string into a list of floats
                historical_prices = [float(price.strip()) for price in prices_str.split(',')]
                
                # Ensure we have exactly 24 hours of data
                if len(historical_prices) != 24:
                    continue  # Skip this entry if it doesn't have 24 hours
                
                # Convert to tensor on the same device as new_price
                historical_price_tensor = torch.tensor(
                    historical_prices, 
                    dtype=torch.float32, 
                    device=new_price.device
                )
                
                # Calculate Euclidean distance between price profiles
                distance = torch.norm(new_price - historical_price_tensor).item()
                
                if distance < min_distance:
                    min_distance = distance
                    closest_belgium_date = belgium_date_str
                    matched_training_date = training_date_str
                    
            except (ValueError, AttributeError) as e:
                # Skip entries with malformed price data
                print(f"Warning: Could not parse prices for date {belgium_date_str}: {e}")
                continue
        
        if matched_training_date is None:
            raise ValueError("No valid historical dates found in Belgium data")
        
        # Convert the matched training date to the expected format (YYYY-MM-DD)
        try:
            # Parse the matched date and reformat if necessary
            parsed_date = datetime.strptime(matched_training_date, '%Y-%m-%d')
            formatted_training_date = parsed_date.strftime('%Y-%m-%d')
        except:
            # If parsing fails, use the date as-is
            formatted_training_date = str(matched_training_date)
        
        # Verify the matched training date exists in historical_data if provided
        if historical_data and formatted_training_date not in historical_data:
            print(f"Warning: Matched training date {formatted_training_date} not found in historical data")
            print(f"Available dates: {list(historical_data.keys())[:5]}...")  # Show first 5 dates
            
            # Fallback: find the closest available date in historical_data
            if historical_data:
                available_dates = list(historical_data.keys())
                # Use the first available date as fallback
                formatted_training_date = available_dates[0]
                print(f"Using fallback date: {formatted_training_date}")
        
        print(f"Closest Belgium date: {closest_belgium_date} -> Training date: {formatted_training_date} (distance: {min_distance:.2f})")
        
        return formatted_training_date, min_distance
        
    except FileNotFoundError:
        print(f"Error: Belgium historical data file not found at {belgium_file_path}")
        raise
    except Exception as e:
        print(f"Error loading Belgium historical data: {e}")
        raise
...
def comprehensive_validation():
    ...
    master_dir = Path("./custom_validation_results/comprehensive")
        ...
        try:
            ...
            for date_idx, (new_date, new_price) in enumerate(new_price_data.items()):
                ...
                try:
                    ...
                    print(f"Using training date: {closest_date} (Belgium price similarity distance: {distance:.2f})")
                    
                    # 2. Look for the pretrained model at this path
                    model_path = Path(f"./trained_models_euclidean/{db_name}/{config_name}/{closest_date}/best_model.pt")
                    
                    if not model_path.exists():
                        # Try model.pt if best_model.pt doesn't exist
                        model_path = Path(f"./trained_models_euclidean/{db_name}/{config_name}/{closest_date}/model.pt")
...
def generate_comprehensive_summary(master_benchmark_file):
    ...
    try:
        ...
        # Create output directory
        summary_dir = Path("./custom_validation_results/comprehensive/summary")
